...
class WanFunControlPipelineGeoWorldstage2(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        super().__init__()
        # vggt embedding
        #self._execution_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, transformer=transformer, clip_image_encoder=clip_image_encoder, vggt=vggt, scheduler=scheduler
        )
    ...
    def __call__(
        ...
        if control_camera_video is not None:
            ...
            #print(control_video_latents.size())
        ...
        if clip_image is not None:
            # clip embedding
            clip_image_ori = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(device, weight_dtype) 
            clip_context = self.clip_image_encoder([clip_image_ori[:, None, :, :]])
            # vggt embedding
            vggt_tensors = []
            for img in vggt_video:
                target_size = 518  
                ...
                vggt_tensors.append(img_tensor)
            vggt_pixel_values = torch.stack(vggt_tensors)
            print("[DBG] now call vggt aggregator")
            vggt_feature, _ = self.vggt.aggregator(vggt_pixel_values.to(device, weight_dtype).unsqueeze(0))
            vggt_feature = vggt_feature[-1]
            vggt_feature = vggt_feature.squeeze(dim=1)
            vggt_context = vggt_feature[..., 5:, :]
        ...
        #print(clip_context.size(), vggt_context.size())
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(control_camera_latents_input.size(),start_image_latentes_conv_in_input.size())
                #control_latents_input = torch.cat([control_camera_latents_input, start_image_latentes_conv_in_input], dim = 1) if control_latents_input is None else \
                #    torch.cat([control_latents_input, start_image_latentes_conv_in_input], dim = 1)
                ...
                vggt_context_input = (
                    torch.cat([vggt_context] * 2) if do_classifier_free_guidance else vggt_context
                )
                ...
                #print(control_latents_input.size())
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=device):
                    noise_pred = self.transformer(
                        ...
                        full_ref=full_ref,
                        clip_fea=vggt_context_input,
                        clip_ori = clip_context_input,
                    )
