...
from videox_fun.models import WanTransformer3DModelGeoWorldstage2 as WanTransformer3DModelVGGT
...
def log_validation(vae, text_encoder, tokenizer, clip_image_encoder, transformer3d, args, config, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_val = WanTransformer3DModelVGGT.from_pretrained(
...
def main():
    ...
    # Get Transformer
    transformer3d = WanTransformer3DModelVGGT.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
    ).to(weight_dtype)
    ...
    if args.train_mode != "normal":
        ...
        # vggt embedding : get vggt model
        vggt = VGGT.from_pretrained("path to /vggt/ckpt")
    ...
    if args.train_mode != "normal":
        ...
        #vggt.train()  # 设置为训练模式，但参数已经requires_grad=False，所以不会更新
    ...
    if args.use_ema:
        ...
        ema_transformer3d = WanTransformer3DModelVGGT.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
            transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
        ).to(weight_dtype)

        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=WanTransformer3DModelVGGT, model_config=ema_transformer3d.config)
        ...
        if not zero_stage == 3:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = WanTransformer3DModelVGGT.load_config(ema_path, return_unused_kwargs=True)
                    load_model = WanTransformer3DModelVGGT.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs'])
                    )
                    load_model = EMAModel(load_model.parameters(), model_cls=WanTransformer3DModelVGGT, model_config=load_model.config)
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            if args.train_mode != "control":
                ...
                # vggt embedding
                new_examples["vggt_pixel_values"] = []
            ...
            for example in examples:
                ...
                #print("control_pixel_values:", control_pixel_values.size())
                ...
                if args.train_mode != "control":
                    ...
                    # vggt embedding
                    #print("new_examples[pixel_values][-1]:", new_examples["pixel_values"][-1].size())#, new_examples["pixel_values"].size())
                    #print("new_examples[control_pixel_values][-1]:", new_examples["control_pixel_values"][-1].size())
                    #for i in new_examples["pixel_values"]:
                    #    print("i:",i.size())
                    #print("000:", new_examples["pixel_values"][-1][clip_index].size())
                    #for i in new_examples["control_pixel_values"]:
                    #    print("j:",i.size())
                    #print("111", new_examples["control_pixel_values"][-1][clip_index].size())
                    vggt_pixel_values = new_examples["control_pixel_values"][-1].permute(0, 2, 3, 1).contiguous()  # shape: (num_frames, height, width, channels)
                    vggt_pixel_values = (vggt_pixel_values * 0.5 + 0.5) * 255

                    # 处理每一帧
                    processed_frames = []
                    for frame_idx in range(vggt_pixel_values.shape[0]):
                        frame = Image.fromarray(vggt_pixel_values[frame_idx].byte().cpu().numpy())
                        ...
                        # Set width to target_size, adjust height maintaining aspect ratio
                        new_width = target_size
                        new_height = int(original_height * (target_size / original_width))
                        # Make height divisible by 14
                        ...
                        if new_height > target_size:
                            ...
                            frame = frame[:, start_y:start_y+target_size, :]
                        
                        processed_frames.append(frame)

                    # 堆叠处理后的帧
                    vggt_pixel_values = torch.stack(processed_frames)  # shape: (num_frames, channels, height, width)
                    new_examples["vggt_pixel_values"].append(vggt_pixel_values)
            ...
            if args.train_mode != "control":
                ...
                # vggt embedding
                new_examples["vggt_pixel_values"] = torch.stack([example[:batch_video_length] for example in new_examples["vggt_pixel_values"]])
    ...
    if args.train_mode != "normal":
        ...
        # vggt embedding
        vggt.to(accelerator.device if not args.low_vram else "cpu", dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        train_vggt_loss = 0.0
        # vggt camera loss logging  
        train_vggt_camera_loss = 0.0
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.train_mode != "control":
                    ...
                    vggt_pixel_values = batch["vggt_pixel_values"]
                    ...
                    if args.training_with_video_token_length and zero_stage != 3:
                        if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            ...
                            # vggt embedding
                            vggt_pixel_values = torch.tile(vggt_pixel_values, (4, 1, 1, 1, 1))
                            ...
                            # vggt embedding
                            vggt_pixel_values = torch.tile(vggt_pixel_values, (2, 1, 1, 1, 1))
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "control":
                        ...
                        vggt_context = []
                        for vggt_pixel_value in vggt_pixel_values:
                            #print("vggt_pixel_value:", vggt_pixel_value.size())
                            #print("control_pixel_values", control_pixel_values.size())
                            vggt_feature, _ = vggt.aggregator(vggt_pixel_value.to(clip_image_encoder.device, weight_dtype).unsqueeze(0))
                            vggt_feature = vggt_feature[-1]
                            vggt_feature = vggt_feature.squeeze(dim=1)
                            #print(vggt_feature.size()) # 1, 19, 782, 2048
                            vggt_feature = vggt_feature[..., 5:, :]
                            #print(vggt_feature.size())
                            vggt_context.append(vggt_feature)
                        vggt_context = torch.cat(vggt_context)

                        #vggt_context = torch.cat((vggt_context, clip_context), dim=-2)
                        #print(vggt_context.size())
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype):
                    noise_pred = transformer3d(
                        ...
                        clip_fea=vggt_context if args.train_mode != "control" else None,
                        clip_ori = clip_context if args.train_mode != "control" else None,
                ...
                def decode_latents(self, latents: torch.Tensor) -> torch.Tensor:
                    frames = vae.to(accelerator.device).decode(latents.to(vae.dtype)).sample
                ...
                # vae process
                with torch.no_grad():
                    gt_frames = decode_latents(vae, latents)
                    pred_frames = decode_latents(vae, noisy_latents - sigmas * noise_pred)

                print("pred_frames.size():", pred_frames.size())



                def process_for_vggt(video_frames):
                    # c f h w
                    video_frames = video_frames.permute(1, 0, 2, 3) # f c h w
                    processed_frames = []
                    for frame_idx in range(video_frames.size()[0]): # f
                        frame = video_frames[frame_idx].to(torch.float32)
                        ...
                        frame = frame.resize((new_width, new_height), Image.Resampling.BICUBIC)
                        frame = TF.to_tensor(frame).to(weight_dtype)
                        ...
                        processed_frames.append(frame)
                    return torch.stack(processed_frames) # return : f c h w 
                
                def process_for_vggt_batch(video_frames):
                     # c f h w
                    video_frames = video_frames.permute(1, 0, 2, 3)
                    target_size = 518
                    _, original_width, original_height = video_frames[0].size()
                    # Set width to target_size, adjust height maintaining aspect ratio
                    new_width = target_size
                    new_height = int(original_height * (target_size / original_width))
                    # Make height divisible by 14
                    new_height = (new_height // 14) * 14
                    processed_frames = []
                    resized_feames = F.interpolate(video_frames, size=(new_height, new_width), mode="bicubic")
                    if new_height > target_size:
                        start_y = (new_height - target_size) // 2
                        resized_feames = resized_feames[:, :, start_y:start_y+target_size, :]
                    return resized_feames

                # vggt process
                gt_vggt_out = []
                pred_vggt_out = []
                #print("gt_frames.size(), pred_frames.size():", gt_frames.size(), pred_frames.size()) # 2 3 17 384 672
                gt_frames_processed = []
                pred_frames_processed = []
                for i in range(gt_frames.size()[0]):
                    # b : c f h w
                    gt_frames_processed.append(process_for_vggt(gt_frames[i]))
                    pred_frames_processed.append(process_for_vggt(pred_frames[i]))
                gt_frames_processed = torch.stack(gt_frames_processed)
                pred_frames_processed = torch.stack(pred_frames_processed)
                # for i in range(gt_frames_processed.size()[0]):
                #     gt_vggt_feature, _ = vggt.aggregator(gt_frames_processed[i].to(clip_image_encoder.device, weight_dtype).unsqueeze(0))
                #     gt_vggt_feature = gt_vggt_feature[-1]
                #     gt_vggt_out.append(gt_vggt_feature)
                #     pred_vggt_feature, _ = vggt.aggregator(pred_frames_processed[i].to(clip_image_encoder.device, weight_dtype).unsqueeze(0))
                #     pred_vggt_feature = pred_vggt_feature[-1]
                #     pred_vggt_out.append(pred_vggt_feature)
                #print(gt_frames_processed.device, clip_image_encoder.device, weight_dtype, accelerator.device)
                vggt.aggregator._resnet_mean = vggt.aggregator._resnet_mean.to(accelerator.device)
                vggt.aggregator._resnet_std = vggt.aggregator._resnet_std.to(accelerator.device)
                #with torch.set_grad_enabled(True): 
                with torch.no_grad():
                    gt_vggt_feature, _ = vggt.aggregator(gt_frames_processed.to(accelerator.device))
                gt_vggt_feature = gt_vggt_feature[-1]
                #gt_vggt_out.append(gt_vggt_feature)
            
                #with torch.no_grad():#important
                pred_vggt_feature, _ = vggt.aggregator(pred_frames_processed.to(accelerator.device))
                pred_vggt_feature = pred_vggt_feature[-1]
                #pred_vggt_out.append(pred_vggt_feature)
                #gt_vggt_out = torch.cat(gt_vggt_out)
                #pred_vggt_out = torch.cat(pred_vggt_out)

                # cal vggt loss
                #print(weight_dtype)
                #print(gt_vggt_out.size(), pred_vggt_out.size()) # 2 17 782 2048

                #print(gt_vggt_feature.size(), pred_vggt_feature.size())

                # 在计算vggt_loss之前检查noise_pred的梯度
                #print(f"noise_pred requires_grad: {noise_pred.requires_grad}")
                #print(f"noise_pred.grad: {noise_pred.grad}")

                vggt_loss_scale = 0.2
                vggt_loss = F.mse_loss(gt_vggt_feature[..., 5:, :].float(), pred_vggt_feature[..., 5: , :].float(), reduction='none')
                vggt_loss = vggt_loss.mean(dim=[2,3])
                vggt_loss = vggt_loss.mean()

                #print(f"vggt_loss requires_grad: {vggt_loss.requires_grad}")

                vggt_camera_loss_scale = 0.1
                vggt_camera_loss = F.mse_loss(gt_vggt_feature[..., 0:1, :].float(), pred_vggt_feature[..., 0:1 , :].float(), reduction='none')
                vggt_camera_loss = vggt_camera_loss.mean(dim=[2,3])
                vggt_camera_loss = vggt_camera_loss.mean()
                ...
                #print(f"mse_loss requires_grad: {loss.requires_grad}")

                print("mse_loss:", loss, "vggt_loss:", vggt_loss, "vggt_camera_loss:", vggt_camera_loss)

                loss = loss + vggt_loss_scale * vggt_loss #+ vggt_camera_loss_scale * vggt_camera_loss
                ...
                # vggt loss logging
                avg_vggt_loss = accelerator.gather(vggt_loss.repeat(args.train_batch_size)).mean()
                train_vggt_loss += avg_vggt_loss.item() / args.gradient_accumulation_steps

                # vggt camera loss logging
                avg_vggt_camera_loss = accelerator.gather(vggt_camera_loss.repeat(args.train_batch_size)).mean()
                train_vggt_camera_loss += avg_vggt_camera_loss.item() / args.gradient_accumulation_steps

                # Backpropagate
                accelerator.backward(loss)
                #print(f"noise_pred.grad after backward: {noise_pred.grad is not None}")
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log({"train_vggt_loss": train_vggt_loss}, step=global_step)
                # vggt camera loss logging
                accelerator.log({"train_vggt_camera_loss": train_vggt_camera_loss}, step=global_step)
