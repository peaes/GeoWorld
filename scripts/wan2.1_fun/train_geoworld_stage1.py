...
from videox_fun.models import WanTransformer3DModelGeoWorldstage1 as WanTransformer3DModelVGGT
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                if args.train_mode != "control":
                    ...
                    # vggt embedding
                    vggt_pixel_values = new_examples["pixel_values"][-1][clip_index].permute(1, 2, 0).contiguous()
                    vggt_pixel_values = (vggt_pixel_values * 0.5 + 0.5) * 255
                    vggt_pixel_values = Image.fromarray(vggt_pixel_values.byte().cpu().numpy())
                    vggt_pixel_values = vggt_pixel_values.convert("RGB")
                    target_size = 518
                    to_tensor = TF.to_tensor
                    original_width, original_height = vggt_pixel_values.size
                    ...
                    vggt_pixel_values = vggt_pixel_values.resize((new_width, new_height), Image.Resampling.BICUBIC)
                    vggt_pixel_values = to_tensor(vggt_pixel_values) 
                    # Center crop height if larger than target_size
                    if new_height > target_size:
                        start_y = (new_height - target_size) // 2
                        vggt_pixel_values = vggt_pixel_values[:, start_y:start_y+target_size, :]
            ...
            if args.train_mode != "control":
                ...
                # vggt embedding
                new_examples["vggt_pixel_values"] = torch.stack([example for example in new_examples["vggt_pixel_values"]])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.train_mode != "control":
                    ...
                    if args.training_with_video_token_length and zero_stage != 3:
                        if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            ...
                            # vggt embedding
                            vggt_pixel_values = torch.tile(vggt_pixel_values, (4, 1, 1, 1))
                            ...
                            # vggt embedding
                            vggt_pixel_values = torch.tile(vggt_pixel_values, (2, 1, 1, 1))
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "control":
                        ...
                        for vggt_pixel_value in vggt_pixel_values:
                            #print(vggt_pixel_value.size(), vggt_pixel_value.unsqueeze(0).unsqueeze(0).size())
                            vggt_feature, _ = vggt.aggregator(vggt_pixel_value.to(clip_image_encoder.device, weight_dtype).unsqueeze(0).unsqueeze(0))
                ...
                loss = loss + vggt_camera_loss_scale * vggt_camera_loss # for stage1, only use camera_loss
